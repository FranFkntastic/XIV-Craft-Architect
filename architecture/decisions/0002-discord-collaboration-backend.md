...
Discord is a projection surface for a canonical Trade company. It does not own Trade orders, choose crafters, or advance work automatically.

The backend binds one active Discord installation to a stable Trade `CompanyId` and posts an ownership-bound immutable commission brief through a durable outbox.
...
- The Discord collaboration store owns installation bindings, message identities, reconciliation cursors, and outbox delivery state.
...
Publication, message projection, and outbox enqueue are idempotent. Discord message IDs are persisted after successful creation; later lifecycle changes edit that exact message.
...
- There is no volunteer-interest flow.

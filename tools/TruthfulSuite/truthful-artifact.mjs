...
const ACCEPTANCE_FILES = ['package-lock.json', 'package.json', 'truth-suite.mjs'];
...
const REQUIRED_OUTCOMES = [
  ...
  { id: 'product-kill-switch', subjectKind: 'archive' },
  { id: 'deterministic-browser-tests', subjectKind: 'archive' }
];
...
function assertEffectiveConfiguration(files, buildManifest) {
  ...
  if (parsed?.ProcurementRoutes?.GenerationEnabled !== false ||
      buildManifest.target.procurementRoutesGenerationEnabled !== false) {
    throw new Error('Procurement route generation must remain disabled.');
  }
...
function validateBuildManifest(manifest) {
  ...
  if (manifest?.target?.procurementRoutesGenerationEnabled !== false) {
    throw new Error('Build manifest enables procurement route generation.');
  }
  ...
  if (JSON.stringify(manifest?.acceptance?.requiredOutcomes) !== JSON.stringify(REQUIRED_OUTCOMES)) {
  ...
  if (manifest?.acceptance?.dotnet?.specTestCases !== 56 ||
      manifest?.acceptance?.dotnet?.contractTestCases !== 66) {
    throw new Error('Build manifest .NET test inventory is incomplete.');
  }
  if (manifest?.acceptance?.worker?.required !== false ||
      manifest?.acceptance?.worker?.status !== 'blocked-source-not-merged') {
    throw new Error('Worker acceptance boundary is not explicit.');
  }
}

async function writeEffectiveConfiguration(root, domain) {
...
export async function createArtifact(options, behavior = {}) {
  ...
  await writeEffectiveConfiguration(root, domain);
  ...
  const buildManifest = {
    ...
    target: { slot, domain, procurementRoutesGenerationEnabled: false },
    ...
    acceptance: {
      ...
      dotnet: { specTestCases: 56, contractTestCases: 66 },
      worker: {
        required: false,
        status: 'blocked-source-not-merged',
        futureOutcomeId: 'worker-acceptance',

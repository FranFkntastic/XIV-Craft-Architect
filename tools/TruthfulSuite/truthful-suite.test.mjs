...
test('terminal verifier accepts only complete outcomes for exact source and archive identities', async () => {
  ...
  const commands = {
    ...
    'product-kill-switch': ['node', '.', ['tools/TruthfulSuite/check-product.mjs', 'dist/subject/src/FFXIV Craft Architect.Web/wwwroot', 'example.com'], 60],
  ...
  await writeFile(contractTrx, passingTrx(66));
  ...
  const productPath = path.join(evidenceDir, 'product-kill-switch.json');
  ...
  await assert.rejects(() => verifyOutcomes(verifyOptions), /wrong command identity: product-kill-switch/);
});

...
public sealed class ExperimentalProcurementEngineRegistrationTests
{
    ...
    public void DefaultCompositionRejectsBeforeBrowserInterop()
    {
        ...
        using var provider = CreateProvider(runtime.Object, executionEnabled: false);
    ...
    {
        var runtime = new Mock<IJSRuntime>(MockBehavior.Strict);
        using var provider = CreateProvider(runtime.Object, executionEnabled: true);
        using var scope = provider.CreateScope();
        var services = scope.ServiceProvider;
        var factory = services.GetRequiredService<ExperimentalProcurementEngineFactory>();
    ...
    public void DeploymentConfigurationEnablesEngineOnlyForLocalDev()
    {
        ...
        Assert.Contains("generation_enabled=\"true\"", workflow, StringComparison.Ordinal);
        Assert.Contains("engine_enabled=\"true\"", workflow, StringComparison.Ordinal);
        Assert.Contains("generation_enabled=\"false\"", workflow, StringComparison.Ordinal);
        Assert.Contains("engine_enabled=\"false\"", workflow, StringComparison.Ordinal);
        Assert.Contains(
            "\"GenerationEnabled\": ${{ steps.deploy-slot.outputs.generation_enabled }}",
            workflow,
            StringComparison.Ordinal);
        Assert.Contains(
            "\"ExecutionEnabled\": ${{ steps.deploy-slot.outputs.engine_enabled }}",
            workflow,
            StringComparison.Ordinal);
    }
